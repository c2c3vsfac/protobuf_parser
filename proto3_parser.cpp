...
std::pair<uint64_t, uint8_t> varint(char * & byte_str)
{   
    uint8_t offset = 0;
    uint64_t data = *byte_str & 0b1111111; // 待优化，要是头铁真塞一个uint64的数据，那结果就错了
    while (true) {
        if (*byte_str >> 7) {
            offset += 1;
            byte_str += 1;
            data = ((*byte_str & 0b1111111) << (7 * offset)) | data;
        }
        else {
            break;
        }
...
template <typename T>
T bytes_to_num_le(const char* byte_str)
{
    T value;
    std::memcpy(&value, byte_str, sizeof(T));
    if (is_big_endian)
    {
...
std::variant<Json::Value, std::pair<int, Json::Value>> parse(char*& byte_str, const int& max_length, const std::string& packet_id, const int& flag, Json::Value encoding_rules, Json::Value prop_names){
    ...
    while (i < max_length){ 
        ...
        if (flag == 3) {
            ...
            data_type = *byte_str & 0b111;
            std::pair<uint64_t, uint8_t> varint_res = varint(byte_str);
            data_id = std::to_string(varint_res.first >> 3);
            byte_str += 1; // varint移过了
            i += varint_res.second;
            i += 1;
        ...
        if (encoding_rules.isMember(data_id)){
            if (data_type == 0){
                ...
                if (encode_type == "double") {
                    decode_data[prop_name] = bytes_to_num_le<double>(byte_str);
                }
                else if (encode_type == "sfixed64") {
                    uint64_t num = bytes_to_num_le<uint64_t>(byte_str);
                    int64_t data = (num >> 1) ^ - static_cast<int64_t>(num & 1);
                    decode_data[prop_name] = data;
                }
                else if (encode_type == "fixed64") {
                    decode_data[prop_name] = bytes_to_num_le<uint64_t>(byte_str);
                }
                i += 8;
                byte_str += 8;
            }
                ...
                if (encode_type == "float") {
                    decode_data[prop_name] = bytes_to_num_le<float>(byte_str);
                }
                else if (encode_type == "sfixed32") {
                    uint32_t num = bytes_to_num_le<uint32_t>(byte_str);
                    int32_t data = (num >> 1) ^ - static_cast<int32_t>(num & 1);
                    decode_data[prop_name] = data;
                }
                else if (encode_type == "fixed32") {
                    decode_data[prop_name] = bytes_to_num_le<uint32_t>(byte_str);;
                }
                i += 4;
                byte_str += 4;
            }
                ...
                if (encoding_rules[data_id].isString()){
                    ...
                    if (encode_type == "string") {
                        ...
                        std::string sub_str (byte_str, byte_str + length);
                        decode_data[prop_name] = sub_str;
                        byte_str += length;
                    }
                    else if (encode_type == "bytes") {
                        prop_name = prop_names[data_id].asString();
                        decode_data[prop_name] = Base64::base64_encode(byte_str, length);
                        byte_str += length;
                    }
                        ...
                        {
                            while (j < i + length) {
                                std::pair<int, Json::Value> repeated_data = std::get<std::pair<int, Json::Value>>(parse(byte_str, length, packet_id, 3, repeated_encoding_rules, repeated_prop_names));
                    ...
                    if (encoding_rules[data_id].isMember("map")){
                        ...
                        Json::Value map_data = std::get<Json::Value>(parse(byte_str, length, packet_id, 1, type_map, map_private_prop_name));
                        ...
                        Json::Value repeat_data = std::get<Json::Value>(parse(byte_str, length, packet_id, 1, repeat_encoding_rules, repeat_prop_names));
                        if (!decode_data.isMember(prop_name)) {
                            decode_data[prop_name] = Json::Value(Json::arrayValue);
                        }
                    ...
                    Json::Value list_data = std::get<Json::Value>(parse(byte_str, length, packet_id, 1, encoding_rules[data_id][0], encoding_rules[data_id][1]));
...
char* parser(char* byte_str, int length, const char* packet_id) {
    if (not initialize){
        init();
        initialize = true;
    }
    //std::string str = { byte_str, static_cast<size_t>(length) };
    std::variant parse_result = parse(byte_str, length, packet_id, 0, {}, {});
...
//int main() {
//    std::ifstream file("long_string3.bin", std::ios::binary);
//
...
//    
//    //const char* result = parser(fileContent.data(), fileContent.length(), "102");
//    char* result = parser(fileContent.data(), fileContent.length(), "1494");
